...
FUNCTION morlet, $ ;*********************************************** MORLET
	...
	norm = sqrt(TOTAL(ABS(morlet)^2,/DOUBLE)/n)
	morlet = morlet/norm
	...
	;PRINT,scale,n,TOTAL(ABS(morlet)^2,/DOUBLE)
...
;....construct time series to analyze, pad if necessary
	...
	IF KEYWORD_SET(pad) THEN BEGIN   ; pad with extra zeroes, up to power of 2
		...
		;ext = 2L^(base2 + 1) - n
		;ypad = [ypad,ypad[n-1] + (ypad[0] - ypad[n-1])*findgen(ext)/ext]
		n = N_ELEMENTS(ypad)
	ENDIF
...
;....construct wavenumber array used in transform [Eqn(5)]
	k = (DINDGEN(n/2) + 1)*(2*!DPI)/(DOUBLE(n)*dt)
